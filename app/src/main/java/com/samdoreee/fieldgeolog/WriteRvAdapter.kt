...
class WriteRvAdapter(val context: Context, val recordList: ArrayList<Record>) :
    RecyclerView.Adapter<WriteRvAdapter.Holder>() {
    ...
    inner class Holder(itemView: View?) : RecyclerView.ViewHolder(itemView!!) {
        ...
        fun bind(record: Record, context: Context) {
            ...
            if (record.project_thumbnail != "") {
                val resourceId = context.resources.getIdentifier(record.project_thumbnail,
                    "drawable",
                    context.packageName)
